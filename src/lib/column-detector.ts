...
export type PlatformType = 'AMAZON_SELLER' | 'SHOPIFY' | 'EBAY' | 'ETSY' | 'WOOCOMMERCE' | 'GENERIC' | 'UNIVERSAL' | 'UNKNOWN'
...
export const PLATFORM_PATTERNS = {
  ...
  SHOPIFY: {
    ...
    patterns: {
      ...
      addressLine1: ['shipping_address1', 'shipping address1', 'shipping_street', 'shipping street'],
    ...
    for (const [fieldKey, patterns] of Object.entries(platform.patterns)) {
      ...
      for (const pattern of patterns) {
        for (let i = 0; i < normalizedHeaders.length; i++) {
          ...
          if (similarity > bestScore && similarity >= 60) { // Seuil minimum de 60%
...
export function analyzeFileStructure(content: string): DetectionResult {
  ...
  const hasHeaders = potentialHeaders.some(header => 
    isNaN(Number(header)) && header.length > 2
  )
