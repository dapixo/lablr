...
/**
 * @deprecated Utiliser printAddresses à la place
 * Crée une fenêtre d'impression avec le contenu et les styles spécifiés
 */
export function createPrintWindow(
  ...
  printWindow.document.write(`
    ...
    <html>
      <head>
        <meta charset="utf-8">
        <title>Impression des adresses - Lablr</title>
        <style>
          * {
            margin: 0;
            padding: 0;
...
/**
 * @deprecated Utiliser printAddresses à la place
 * Lance l'impression avec un délai pour s'assurer que le contenu est chargé
 */
export function triggerPrint(printWindow: Window, delay: number = PRINT_DELAY_MS): void {
  setTimeout(() => {
    printWindow.print()
  }, delay)
}