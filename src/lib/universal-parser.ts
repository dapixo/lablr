...
import { analyzeFileStructure, type ColumnMapping, type DetectionResult } from './column-detector'
...
export function parseUniversalFile(content: string): UniversalParseResult {
  ...
  if (lines.length === 0) {
    return {
      addresses: [],
      errors: ['Fichier vide'],
      detection: { mapping: {}, confidence: 0, platform: 'UNKNOWN', separator: ',', hasHeaders: false },
      confidence: 0,
      platform: 'UNKNOWN'
    }
  }
  ...
  for (let i = 0; i < dataLines.length; i++) {
    ...
    try {
      ...
      errors.push(`Erreur ligne ${i + 2}: ${error instanceof Error ? error.message : 'Format invalide'}`)
    }
  }
...
function extractAddressFromColumns(columns: string[], mapping: ColumnMapping): Partial<Address> | null {
  ...
  const shippingStreetCol = mapping.addressLine1 !== undefined ? getColumn(mapping.addressLine1 - 1) : ''
  
  
  // Logique d'extraction intelligente
  if (shippingStreetCol && shippingStreetCol.length > 10 && !shippingStreetCol.includes('Payments')) {
    // Utiliser Shipping Street si elle contient une adresse complète
    data.addressLine1 = cleanAddressField(shippingStreetCol)
    data.addressLine2 = rawAddress2 && cleanAddressField(rawAddress2) !== data.addressLine1 ? cleanAddressField(rawAddress2) : undefined
...
function cleanAddressField(field: string): string {
  ...
  if (/^[\d.]+$/.test(cleaned) || // Que des chiffres et points (shipping cost)
      cleaned.includes('Shopify Payments') || 
      cleaned.includes('Anasalsa Créations') ||
      cleaned.length > 200) { // Trop long = probablement données parasites
    return ''
  }
...
export function parseAmazonSellerReportUniversal(content: string): ParsedAddresses {
  ...
  const { parseAmazonSellerReport } = require('./address-parser')
...
export { analyzeFileStructure } from './column-detector'
