...
import { CheckoutRequest, CheckoutResponse } from '@/types/lemonsqueezy'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const isTestMode = variantId.startsWith('test_') || LEMONSQUEEZY_CONFIG.storeId.startsWith('test_')
    ...
    if (!variantId) {
      const envVar = billingCycle === 'yearly' ? 'LEMONSQUEEZY_VARIANT_YEARLY' : 'LEMONSQUEEZY_VARIANT_MONTHLY'
      return NextResponse.json(
    ...
    const checkoutData = {
      ...
      productOptions: {
        ...
        receiptThankYouNote: 'Merci pour votre abonnement Premium ! Vous pouvez maintenant profiter de toutes les fonctionnalités.'
      },
